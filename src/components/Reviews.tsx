...
const Review = ({imgSrc, className, ...props}: ReviewProps) => {
    ...
    return (
        <div className={cn('animate-fade-in rounded-[2.25rem] bg-white p-6 opacity-0 shadow-xl shadow-slate-900/5', className)}  {...props}
...
function ReviewGrid(){
    ...
    return <div ref={containerRef} className='relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3'>
        ...
        <ReviewColumn reviews={[...column2, ...column3[1]]}
            ...
            msPerPixel={15}
        />
        <ReviewColumn reviews={columns.flat()}
            className='hidden md:block' 
            msPerPixel={10}
        />
        </>
...
const Reviews = () => {
  return (
    <MaxWidthWrapper className='relative max-w-5xl'>
        <img aria-hidden='true' src="/what-people-are-buying.png" alt="review" className='absolute select-none hidden xl:block -left-32 top-1/3' />
        <ReviewGrid/>
    </MaxWidthWrapper>
  )
}
